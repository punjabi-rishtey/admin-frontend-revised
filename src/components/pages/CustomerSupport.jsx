...
const CustomerSupport = () => {
  ...
  const fetchInquiries = async () => {
    try {
      const { data } = await adminApi.fetchInquiries({ status: filter });
  ...
  const handleReply = async (e) => {
    e.preventDefault();
    try {
      await adminApi.replyToInquiry(selectedInquiry._id, { message: replyMessage });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value) => {
        ...
        return (
          <span className={`px-2 py-1 text-xs rounded-full font-medium flex items-center space-x-1 ${config.color}`}>
        ...
        {selectedInquiry && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h4 className="font-medium text-gray-900 mb-2">{selectedInquiry.subject}</h4>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900">{new Date(inquiry.createdAt).toLocaleString()}</p>
              </div>
            </div>
